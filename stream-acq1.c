...
s32 RecvStreamInit1(s32* sfd_rtp, s32* sfd_rtcp, s8* argv, s16* port_rtp, s16* port_rtcp)
{
	struct sockaddr_in cliaddr_rtp, servaddr_rtcp;
        ...
        cliaddr_rtp.sin_port = htons(37436);
...
s32 ReqRTSPOpts1(s32* sfd, s8* argv, s8* param){
	...
	const s8 cmd2[] = ":554/ch0_0.h264 RTSP/1.0\r\nCSeq: 0\r\n\r\n";
	s8 tl[400];
	s8 rl[1500];
	s32 n;
...
s32 GetTimedStream1(s32* sfd, FILE* fptr, u32 time)
{
        ...
        u32 inittime;
        s32 status;
        inittime = GetStream1(sfd, fptr);
        while(rettime < (inittime + (time * 90000))) {
                rettime = GetStream1(sfd, fptr);
        }
        return SUCCESS;
}
